...
|----------|-------------|
| Температура | Степень случайности ответа модели. `0.0` — детерминированный, точный ответ. `2.0` — максимально творческий. Диапазон зависит от модели (обычно `0.0–1.0` для GLM, `0.0–2.0` для GPT). Рекомендуется `0.2` для структурированных задач. |
